...
def load_vectordb(embeddings):
    ...
    langchain_chroma = Chroma(
        ...
        collection_name=config[f"chromadb"]["collection_name"],
...
class chatChain:
    ...
    def run(self, user_input, chat_history):
        response = self.llm_chain.invoke(input={"human_input": user_input, "history": chat_history}, stop=["Human:"])["text"]
        
        print(response)
        # Manually stop at "Human:" if the model doesn't respect the stop parameter
        stop_index = response.find("Human:")
        print(stop_index)
        if stop_index != -1:
            response = response[:stop_index].strip()
